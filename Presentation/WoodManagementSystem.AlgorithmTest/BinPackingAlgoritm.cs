...
{
    public class BinPackingAlgoritm
    {
        private int WhitespaceWeight = 1;
        private int SideLengthWeight = 20000;

        public bool Intersects(OrderDetails a, OrderDetails b)
        {
        ...
        public bool Validate(List<OrderDetails> rects, OrderDetails rect)
        {
            var a = rect;
        ...
        public Bound FindBounds(List<OrderDetails> orderDetails)
        {
            double width = 0;
            double height = 0;
            for (var i = 0; i < orderDetails.Count(); i++)
            {
                var rect = orderDetails[i];
        ...
        public List<Position> FindPositions(List<OrderDetails> orderDetails)
        {
        ...
        public OrderDetails FindBestRect(Layout layout,OrderDetails size)
        {
            var bestRect = new OrderDetails();
        ...
        public List<int> PreOrder(List<OrderDetails> sizes)
        {
        ...
        public List<Layout> Pack(List<OrderDetails> sizes,Pattern pattern, List<Layout> layoutList)
        {
            var layout = new Layout()
            {
                Width = 0,
                Height = 0,
                Rects=new List<OrderDetails>()
            };
            if (sizes.Count() == 0)
            {
                return layoutList;
            ...
            for(var i = 0; i < sizes.Count(); i++)
            {
                ...
                {
                    var tempSizes = new List<OrderDetails>();
    ...
    public class Layout
    {
        ...
        public List<OrderDetails> Rects { get; set; }
    }
    public class Bound
    {
